...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      initPage() {
        ...
        System_API.getMenu_sysBaseData().then(res => {
          SystemApproval_API.findList().then(res1 => {
            ...
            //所有主题流程
            this.allSubjectList = resdata || [];
            //通用表单主题
            this.subjectList = resdata.filter(item => {
              return item.formType == '通用表单';
            })
            ...
            this.menuDataList.forEach(item => {
              switch (item.text) {
                case "月工资发放审核":
                  this.gradeCategoryList = item.subList || [];
                  break;
                case "个人所得税计算":
                  this.taxCompanyList = item.subList || [];
                  break;
                case "预录用":
                  this.hireCompanyList = item.subList || [];
                  break;
              }
            })
            ...
            console.log(err1);
            this.$message({type: 'error', message: '通用表单查询异常：' + err1.msg})
          })
        }).catch(err => {
          console.log(err);
      ...
      reCombingMenu(menusRouterData) {
        var menudData = [];
        menusRouterData.forEach((item, index) => {
          if (item.children) {
            if (item.meta && item.meta.childrenDataFrom) {
              switch (item.meta.childrenDataFrom) {
                case 'gradeApi':
                  //职等
                ...
                case 'allSubjectApi':
                  //公文主题
                  item.path = item.children[0].path;//取第一行的数据
                  item.children = this.menuCombine(this.allSubjectList, item.path);
                  break;
                case 'subjectApi':
                  //公文主题
                  item.path = item.children[0].path;//取第一行的数据
                  item.children = this.menuCombine(this.subjectList, item.path);
                  break;
              }
            }
          ...
          //根据用户校验是否属于显示
          if (item.meta && item.meta.checkUser) {
            item.hidden = window.global_config.FLOW_NOT_START_USER.indexOf(this.login_user.name)>-1 ? true : false;
          }
          menudData.push(item);
    ...
    mounted() {
      //流程 （流程引擎的页面包含 /approval）
      if (window.global_config.IS_FLOW) {
        ...
        this.menusRouterData = router || [];
        this.initPage();
      }
      else {
        this.menusRouterData = this.$router.options.routes.splice(2);
        this.menusRouterData = this.menusRouterData.filter(item => {
          return item.path.indexOf('/approval') === -1;
        }) || [];
        this.initPage();
      }
