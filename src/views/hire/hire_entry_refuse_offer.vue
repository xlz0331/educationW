<template>
  <div>
    ...
    <el-row>
      ...
      <el-col :span="2">
        <div style="height:30px;background:#f5f7fa">
          <el-input v-model="name" @keyup.enter.native="findAllRefuseOffer()"></el-input>
        </div>
      </el-col>
      <el-col :span="2" style="height:30px;background:#f5f7fa;">
        <div style="margin-top:3.5px">
      ...
      <el-col :span="2">
        <div style="height:30px;background:#f5f7fa">
          <el-input v-model="dept" @keyup.enter.native="findAllRefuseOffer()"></el-input>
        </div>
      </el-col>
      <el-col :span="2" style="height:30px;background:#f5f7fa;">
        <div style="margin-top:3.5px">
      ...
      <el-col :span="2">
        <div style="height:30px;background:#f5f7fa">
          <el-input v-model="jobGrade" @keyup.enter.native="findAllRefuseOffer()"></el-input>
        </div>
      </el-col>
      <el-col :span="2" class="text-left">
        <el-button type="primary" @click="findAllRefuseOffer()" style="margin-left:10px">查询</el-button>
      </el-col>
    ...
    <div @click="addrepay()">
      <el-table :data="tableData">
        ...
        <el-table-column prop header-align="center" label="入职借款及欠条" :render-header="renderheader">
          ...
          >
            <template slot-scope="scope">
              ...
              <!-- <span style="cursor:pointer;color:#409EFF;text-decoration:underline" @click="proofOut1(scope.row)" v-html="scope.row.borrowText"></span> -->
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column prop header-align="center" label="入职薪酬确认书" :render-header="renderheader">
          ...
          >
            <template slot-scope="scope">
              <!-- <span style="cursor:pointer;color:#409EFF;text-decoration:underline" v-html="scope.row.salaryConfirmText" @click="goSalaryInput(scope.row)">已存档</span> -->
    ...
    <div>
      <el-dialog :close-on-click-modal="false" :visible.sync="dialogBorrow" width="840px">
        <div id="bb">
          <el-row>
...
export default {
  ...
  methods: {
        ...
        if (comList.length > 0) {
          for (var i = 0; i < comList.length; i++) {
            ...
            if (this.companyIdA == comList[i].companyId) {
              this.companyName = comList[i].companyName + "社招预注册管理人员【入职薪酬已确认不入职】明细表";
            }
          }
        }
      }).catch(err => {
    ...
    findAllRefuseOffer() {
      var companyId = this.companyIdA;
      this.tableData = [];
      var params = { companyId: companyId, name: this.name, dept: this.dept, jobGrade: this.jobGrade }
      Hire_API.findAllRefuseOffer(params).then(res => {
